package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1638:uint = 1;
      
      protected static const const_1639:uint = 2;
      
      protected static const const_1641:uint = 4;
      
      protected static const const_1640:uint = 3;
      ...
      public static const const_1141:Boolean = false;
      
      public static const const_1629:int = 2;
      
      private static var var_957:Array = new Array();
      
      private static var var_773:Array = new Array();
      ...
      protected var var_223:URLRequest;
      
      protected var var_644:XML;
      
      protected var var_530:Class;
      
      protected var var_431:uint = 0;
      
      protected var _name:String;
      
      protected var var_973:Boolean = false;
      
      protected var _debug:Boolean = false;
      
      protected var var_974:Boolean = false;
      
      protected var var_2723:String = "";
      
      protected var var_2721:String = "";
      
      protected var var_1972:int;
      
      protected var var_1220:int;
      
      protected var var_1221:int;
      
      protected var var_2792:uint = 0;
      
      protected var var_2722:Boolean = true;
      ...
      protected static function method_8(param1:LibraryLoader) : void
      {
         if(const_1141)
         {
            if(var_957.indexOf(param1) == -1)
            {
               var_957.push(param1);
            }
            throttle();
         ...
         {
            param1.var_39.load(param1.var_223,param1.var_15);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1141)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_773[_loc2_] as LibraryLoader;
            ...
            while(var_773.length < const_1629 && false)
            {
               _loc1_ = var_957.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_773.push(_loc1_);
                  _loc1_.var_39.load(_loc1_.var_223,_loc1_.var_15);
               }
            }
         }
      }
      ...
      {
         return !!this.var_223 ? this.var_223.url : null;
      }
      ...
      {
         return !!this.var_973 ? uint(this.var_1220 - this.var_1972) : uint(getTimer() - this.var_1972);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_370,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_39.close();
            }
      ...
      public function load(param1:URLRequest, param2:int = 5) : void
      {
         ...
         this._name = parseNameFromUrl(this.var_223.url);
      ...
      {
         if(this.var_974 && !_disposed)
         {
            this.var_974 = false;
            if(!this.var_973 && this.var_223)
            {
               this.var_1220 = -1;
               this.var_1972 = getTimer();
               method_8(this);
            }
         }
      }
      ...
      {
         if(!this.var_973 && !_disposed && this.var_1221 > 0)
         {
            try
            {
               this.var_39.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_223,const_1482 - this.var_1221);
            if(this.var_2722)
            {
               this.var_223.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_223.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2722 = false;
            }
            --this.var_1221;
            this.var_431 = 0;
            this.var_39.load(this.var_223,this.var_15);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_370 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_370,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_370,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_370))
               {
                  this.var_1220 = getTimer();
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_431 = LibraryLoader.name_2;
         ...
         if(this.var_431 == LibraryLoader.const_1640)
         {
         ...
         if(this.var_431 == LibraryLoader.name_2)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_370,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_530 = this.getDefinition(this._name) as Class;
         if(this.var_530 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_223.url + "!");
            }
            return false;
         }
         try
         {
            xmlClass = this.var_530.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
         ...
         try
         {
            this.var_644 = new XML(bytes.readUTFBytes(bytes.length));
         }
         catch(e:Error)
         {
            if(!retry())
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_370,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_370,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
