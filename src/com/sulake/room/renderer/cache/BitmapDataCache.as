package com.sulake.room.renderer.cache
{
   ...
   public class BitmapDataCache
   {
      ...
      public function compress() : void
      {
         ...
         {
            this.var_378.sortOn("useCount",0 | 0);
            _loc2_ = this.var_378.length - 1;
            while(_loc2_ >= 0)
            {
               _loc1_ = this.var_378[_loc2_] as BitmapDataCacheItem;
            ...
            this.var_378.splice(_loc2_,this.var_378.length - _loc2_);
            if(this.memUsage > this.memLimit)
            {
               this.var_378.sortOn("memUsage",Array.NUMERIC);
               while(this.memUsage > this.memLimit)
               {
                  _loc1_ = this.var_378.pop() as BitmapDataCacheItem;
                  if(_loc1_ == null)
                  {
                     break;
                  }
