package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1150:IContainerButtonWindow;
      
      private var var_1151:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_90.getXmlWindow("ude_help"));
         this.var_1807 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1151 = this.var_1807.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1151)
         {
            this.var_1151.procedure = this.onButtonClick;
         }
         this.var_1150 = this.var_1807.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1150)
         {
            this.var_1150.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_257 >= this._localizations.length - 1)
         {
            this.var_1150.disable();
         }
         else
         {
            this.var_1150.enable();
         }
         if(this.var_257 <= 0)
         {
            this.var_1151.disable();
         }
         else
         {
            this.var_1151.enable();
         }
      }
      ...
      {
         ++this.var_257;
         this.var_257 = Math.min(this.var_257,this._localizations.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_257;
         this.var_257 = Math.max(this.var_257,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this._localizations = [];
         var _loc1_:int = 1;
         while(this.var_90.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_90.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_90.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_90.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this._localizations[this.var_257][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this._localizations[this.var_257][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this._localizations[this.var_257][1]).value;
      ...
      {
         if(!param1 || !this.var_90 || !this.var_90.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_90.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_90.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_90.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
