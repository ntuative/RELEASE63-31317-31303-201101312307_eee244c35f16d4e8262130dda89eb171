package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_446.numGridItems)
         {
            return;
         }
         this.select(this.var_446.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_446 == null)
         {
            return;
         }
      ...
      private function select(param1:IWindowContainer) : void
      {
         ...
         {
            _loc2_ = this.var_317.getChildByName("chosen");
            if(_loc2_ != null)
            {
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         this.var_446.destroyGridItems();
         this.var_317 = null;
         for each(_loc2_ in this.colors)
         {
            _loc3_ = param1.buildFromXML(this.var_1531) as IWindowContainer;
            ...
            _loc3_.width = this.var_539.width;
            _loc3_.height = this.var_539.height;
            this.var_446.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_539.width,this.var_539.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_539,this.var_539.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_1009.width,this.var_1009.height,true,0);
      ...
      {
         var _loc2_:int = this.var_446.getGridItemIndex(param1.target as IWindow);
