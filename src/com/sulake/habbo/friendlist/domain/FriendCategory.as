package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         if(this.var_963 >= this.getPageCount())
         {
            this.var_963 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_221.length / const_342);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_963 * const_342;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_963 + 1) * const_342,this.var_221.length);
      }
