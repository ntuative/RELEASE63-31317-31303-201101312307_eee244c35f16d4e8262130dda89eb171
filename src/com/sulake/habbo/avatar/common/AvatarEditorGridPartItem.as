package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_763:IFigurePartSet;
      ...
      private var var_420:IAvatarRenderManager;
      
      private var var_695:Boolean;
      
      private var var_1865:BitmapData;
      
      private const const_1948:int = 13421772;
      
      private const const_1949:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_642 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1866 = Math.max(this.var_1866,_loc6_.colorLayerIndex);
            }
         }
         this.var_420 = this.var_64.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_642 != null && !this.var_1867)
            {
               _loc3_ = this.var_642;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_763)
            {
               switch(this.var_763.clubLevel)
               {
                  case HabboClubLevelEnum.const_53:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_60:
                     _loc2_.style = HabboIconType.const_850;
                     break;
                  case HabboClubLevelEnum.const_54:
                     _loc2_.style = HabboIconType.const_725;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1948;
         }
         else
         {
            this._window.color = this.const_1949;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_420.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_420.isFigureReady(_loc1_))
         {
            this.var_420.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_327[_loc6_] + "_" + FigureData.const_130;
               _loc4_ = this.var_420.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_622 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1865)
               {
                  ...
                  this.var_1865 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1867 && _loc2_.colorLayerIndex > 0)
               {
